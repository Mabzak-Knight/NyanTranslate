...
from penerjemah_manga.basemodel import TextDetBase, TextDetBaseDNN
from penerjemah_manga.utils.db_utils import SegDetectorRepresenter
from penerjemah_manga.utils.imgproc_utils import letterbox, xyxy2yolo, get_yololabel_strings
from penerjemah_manga.utils.io_utils import imread, imwrite, find_all_imgs, NumpyEncoder
from penerjemah_manga.utils.textblock import TextBlock, group_output, visualize_textblocks
from penerjemah_manga.utils.textmask import refine_mask, refine_undetected_mask, REFINEMASK_INPAINT, \
    REFINEMASK_ANNOTATION
from penerjemah_manga.utils.yolov5_utils import non_max_suppression
...
def model2annotation(model_path, img_dir_list, save_dir, save_json=False):
    ...
    for img_path in tqdm(imglist):
        ...
        blk_xyxy = xyxy2yolo(blk_xyxy, im_w, im_h)
        if blk_xyxy is not None:
            cls_list = list(range(len(blk_xyxy)))
            yolo_label = get_yololabel_strings(cls_list, blk_xyxy)
        else:
            yolo_label = ''
        kordinat_list.append(yolo_label)
